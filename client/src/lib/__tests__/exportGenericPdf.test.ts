...
describe("buildGenericPdfDoc", () => {
  ...
  it("includes subtitle with date", () => {
    const doc = buildGenericPdfDoc(
      "Title",
      "Sub",
      ["X"],
  ...
  it("sets font size for header", () => {
    const doc = buildGenericPdfDoc("T", "S", ["A"], [["1"]]);
    expect(doc.setFontSize).toHaveBeenCalledWith(16);
  });

  it("writes total record count in footer", () => {
    const rows = [["a", "b"], ["c", "d"], ["e", "f"]];
    const doc = buildGenericPdfDoc("T", "S", ["X", "Y"], rows);
    const textCalls = (doc.text as ReturnType<typeof vi.fn>).mock.calls;
...
describe("exportGenericPdf", () => {
  it("calls save with filename containing date", () => {
    exportGenericPdf("T", "S", ["A"], [["1"]], "test-file");
